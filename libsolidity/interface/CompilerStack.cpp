...
Json CompilerStack::ethdebugInstructions(Contract const& _contract, bool _runtime) const
{
	...
	solUnimplementedAssert(
		assembly->eofVersion() == std::nullopt && assembly->codeSections().size() == 1,
		"ethdebug does not support eof yet."
	);
	...
	for (size_t i = 0; i < object->offsets.size(); ++i)
	{
		...
		operation["mnemonic"] = instructionInfo(static_cast<evmasm::Instruction>(object->bytecode[offset]), m_evmVersion).name;
		bytes argumentData;
		...
		range["length"] = location.end - location.start;
